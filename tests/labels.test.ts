...
describe('Private equipment labels', () => {
  ...
  it('moves labels with independent history, without moving equipment or changing JSON/model revision', () => {
    ...
    editor.update(key, { kv_rating: 13.8, amp_rating: 1200 })
...
describe('Selected failover relationships', () => {
  it('keeps linked owners marked across selection, history, and reopening a model', () => {
    const e = new Editor()
    ...
    expect(e.hasFailoverLinks(owner)).toBe(false)
    e.setFailover(owner, [], null)
    expect(e.hasFailoverLinks(owner)).toBe(false)
    e.setFailover(owner, [trigger], null)
    e.select([])
    expect(e.hasFailoverLinks(owner)).toBe(true)
    expect(e.hasFailoverLinks(trigger)).toBe(false)
    e.setFailover(owner, [], target)
    e.select([target])
    expect(e.hasFailoverLinks(owner)).toBe(true)
    expect(e.hasFailoverLinks(target)).toBe(false)
    e.setFailover(owner, [trigger], target)
    expect(new Editor(e.snapshot()).hasFailoverLinks(owner)).toBe(true)
  ...
  it('tracks target unions separately and refreshes after edits, undo, deletion and deselection', () => {
    const e = new Editor()
    const a = e.add('indoor_drawout_breaker', { x: 0, y: 0 })
    const b = e.add('load', { x: 300, y: 0 })
    const source = e.add('utility_source', { x: 600, y: 0 })
    const generator = e.add('generator', { x: 900, y: 0 })
    e.setFailover(a, [source], generator)
    e.setFailover(b, [generator], source)
    e.select([a], false)
    expect(e.isSelectedTarget(generator)).toBe(true)
    expect(e.isSelectedTarget(source)).toBe(false)
    e.select([a, b], false)
    expect(e.isSelectedTarget(source)).toBe(true)
    expect(e.isSelectedTrigger(source)).toBe(true)
    expect(e.isSelectedTarget(generator)).toBe(true)
    e.setFailover(a, [source], null)
    expect(e.isSelectedTarget(generator)).toBe(false)
    e.undo()
    expect(e.isSelectedTarget(generator)).toBe(true)
    e.redo()
    expect(e.isSelectedTarget(generator)).toBe(false)
    e.select([source], false)
    e.deleteSelection()
    e.select([b], false)
    expect(e.isSelectedTarget(source)).toBe(false)
    e.undo()
